package baekjoon.coding_test_book_baekjoon.A11_10;
...
민호가 만든 임의의 DNA 문자열과 비밀번호로 사용할 부분분자열의 길이, 그리고 {‘A’, ‘C’, ‘G’, ‘T’} 가 
각각 몇번 이상 등장해야 비밀번호로 사용할 수 있는지 순서대로 주어졌을 때 민호가 만들 수 있는 비밀번호의 종류의 수를 구하는 프로그램을 작성하자.
...
public class A09_DNA12891 {
	...
	public static void main(String[] args) throws IOException{
		...
		char[] dnaArr = new char[dna]; 
		for(int i=0; i<dna; i++) {
			dnaArr[i] = str.charAt(i);
		}
		...
		while(last<dna) {
			if(dnaArr[first]=='A') {
				checkA--;
			}else if(dnaArr[first]=='C') {
				checkC--;
			}else if(dnaArr[first]=='G') {
				checkG--;
			}else {
				checkT--;
			}
			...
			if(dnaArr[last]=='A') {
				checkA++;
			}else if(dnaArr[last]=='C') {
				checkC++;
			}else if(dnaArr[last]=='G') {
				checkG++;
			}else {
				checkT++;
			}
