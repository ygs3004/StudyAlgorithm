...
package javaPackage.solved_class2.S2_2805;
...
public class TreeCut {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(trees[i] > maxHeigt) maxHeigt = trees[i];
        }

        long result = calcCutTree(0, maxHeigt);
    ...
    static long calcCutTree(long minCut, long maxCut){
        if (minCut > maxCut) return maxCut;
        ...
        for(int i=0; i<n; i++){
            if (trees[i] >= midCut){
                totalCut += trees[i] - midCut;
            }
        }

        if (totalCut >= needHeight){
            return calcCutTree(midCut+1, maxCut);
        }else{
            return calcCutTree(minCut, midCut-1);
        }
    }
